...
namespace AO.Network
{
    ...
    {
        public ServerPackets PacketId { get; }
        ...
        /// <summary>Creates a new packet with a given ID. Used for sending.</summary>
        /// <param name="packetId">The packet ID.</param>
        /// <param name="capacity">Reserves this amount of memory for the buffer.</param>
        public Packet(ServerPackets packetId, int capacity)
        {
            buffer = new List<byte>(capacity); // Initialize buffer
            readPos = 0; // Set readPos to 0
        ...
        /// <summary>Tries to read a short from the packet. Used to read the packet id.</summary>
        /// <param name="value">The value read.</param>
        /// <param name="moveReadPos">Whether or not to move the buffer's read position.</param>
        public bool TryReadShort(out short value, bool moveReadPos = true)
        {
            if ((buffer.Count - readPos) >=  sizeof(short))
            {
                value = BitConverter.ToInt16(readableBuffer, readPos);
